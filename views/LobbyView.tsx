...
const LobbyView: React.FC<LobbyViewProps> = ({ onJoinGame, createTrigger, userName, userAvatar }) => {
  ...
  const handleJoinRoom = async (room: GameRoom) => {
    ...
    // Se já sou o criador ou o oponente da sala, entro direto
    ...
    if (room.status !== 'waiting') {
      alert("Esta sala já está em andamento ou cheia.");
      return;
    }

    // Tentar atualizar a sala para entrar como oponente
    // Usamos maybeSingle() para evitar o erro de coerção caso a regra de segurança barre o update
    const { data, error } = await supabase
      .from('rooms')
      .update({
        opponent_id: user.id,
    ...
    if (error) {
      ...
      alert("Não foi possível entrar na sala. Verifique sua conexão ou se a sala ainda existe.");
    } else if (!data) {
      alert("Sala ocupada ou removida. Tente outra!");
      fetchRooms();
    } else {
