...
const LobbyView: React.FC<LobbyViewProps> = ({ onJoinGame, createTrigger, userName, userAvatar }) => {
  ...
  const fetchRooms = async () => {
    const { data, error } = await supabase
      .from('rooms')
      .select('*')
      .or('status.eq.waiting,status.eq.playing') // Mostrar salas esperando ou em jogo (se o usuário já estiver nela)
  ...
  const handleJoinRoom = async (room: GameRoom) => {
    ...
    // Se eu já sou o criador ou o oponente dessa sala, apenas entro
    if (user?.id === room.creator_id || user?.id === room.opponent_id) {
      onJoinGame(room);
      return;
    }

    // Se a sala estiver cheia e eu não for um dos jogadores, não faz nada
    if (room.status !== 'waiting') {
      alert("Esta sala já está em andamento.");
      return;
    }

    // Registrar entrada do oponente
    const { data, error } = await supabase
      .from('rooms')
      .update({
        opponent_id: user?.id,
